...
- [x] SDD 사진 예시를 테스트 fixture로 사용한다. 메타데이터 제거 비활성화, 파일 업로드 단계, 존재하지 않는 inputRef, 중복 입력 키, 잘못된 어댑터 버전, 실행 대상 변경을 각각 거부하는 테스트를 먼저 작성하고 실패를 확인한다.
- [x] `events.ts`에 시작·종료 사이의 정제 이벤트 계약을 작성한다. 순번·상대 시각·등록 소스/필드·허용 값만 받아들이며 사진 입력은 `recipient_file_input` 자리표시자다. 미등록 필드, 파일명/EXIF/인증정보 같은 추가 필드는 거부한다. 이 단계는 실제 기록 확장을 구현하지 않는다.
- [x] `workflow.ts`에 strict 스키마와 레지스트리 검증을 구현한다. SDD의 사진 규칙을 유지하고, 참조와 동작 순서를 검증한다. 공개 조회는 주입된 어댑터 정책을 통해 검증하되 내장 실제 연동을 가장하지 않는다.
- [x] `run.ts`에 허용 전이를 구현한다. QUEUED→RUNNING/CANCELED, RUNNING→종료 상태만 허용한다. 다음 테스트가 취소 후 성공 덮어쓰기를 잡아야 한다.
...
- [x] `pnpm test`와 `pnpm --filter @hanbeonman/contracts typecheck`를 실행한다. 거부·수락 사례가 모두 통과해야 한다.
...
- [x] Next.js App Router의 한국어 시작 화면을 구성한다. 핵심 문구는 “한 번의 도움, 다음부터 스스로.”, 시연 가능한 두 작업 유형과 제작→검토→전달 흐름을 설명한다. 현재 상태를 “개발 중”으로 명시하고 작동하지 않는 생성/로그인 버튼을 배치하지 않는다. 화면 내 이동 링크는 실제 대상이 있어야 한다.
- [x] 모바일 기본 글자 18px, 주요 조작 영역 최소 48px, 명확한 포커스, 한국어 문서 언어를 적용한다. 외부 폰트/이미지/추적 스크립트 요청 없이 실행한다.
- [x] Playwright에서 아래 실제 페이지 검증을 작성한다. 일반적인 정적 문구를 한 줄씩 검사하는 테스트는 추가하지 않는다.
...
- [x] CI는 체크아웃, Node 24·pnpm 설치, frozen lockfile 설치, 타입 검사, 단위 테스트, 프로덕션 빌드, Chromium 설치 및 E2E를 순서대로 실행한다. 권한은 contents: read로 둔다. 클라우드 배포와 secret은 이 CI에 필요 없다.
- [x] 실제 production build와 데스크톱·모바일 Chromium에서 확인한 뒤 `feat: 한국어 웹 시작 화면과 모바일 검증 구현`으로 독립 커밋하고 즉시 푸시한다.

## 검토와 인계

- [x] README에 실제 설치 명령과 계정 준비 순서를 반영한다.
- [x] SDD의 기술 결정과 후속 기능 task 상태를 갱신한다.
- [x] 문서 링크·비밀 값 비포함·git diff --check를 확인한다.
- [x] 두 task를 독립 검토한 뒤 전체 변경을 검토한다. 기존 사용자 승인에 따라 각 기능 완료 시 main에 합치고 한글 커밋으로 각각 푸시한다. 여러 기능을 모아 한 번에 푸시하지 않는다.
