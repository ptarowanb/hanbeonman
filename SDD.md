...
> **한 번의 도움, 다음부터 스스로.**
>
> 자주 하는 작업을 설명하거나 선택하고, 다른 사람도 반복해서 사용할 수 있는 개인용 버튼으로 만든다.

| 항목 | 내용 |
| --- | --- |
| 버전 | 2.1 |
| 작성일 | 2026-09-14 |
| 상태 | 17개 생활 작업·짧은 작업명 질문·템플릿·초안 및 저장 버튼 편집·로컬 보관함·실제 실행 연결·백업·설정 복사 링크 구현 및 통합 검증·Production 주요 흐름 점검 완료. 검증 범위는 16.7절과 17.1절을 따르며 Supabase 인증·동기화·서버 공유 토큰·공개 출시 운영과 실제 사용자 검증은 후속 범위 |
...
**버전 2.1 적용 순서:** 신규 6종과 도구 검색·분류는 [17절](#17-버전-21-생활-버튼-6종-확장)을 우선한다. 그 외 사용자에게 제공하는 동작과 완료 판정에는 [16절](#16-버전-20-로컬-생활-버튼-구현-기준)이 우선한다. 1~14절에 남긴 Chrome 기록, Supabase 소유권·불변 발행 버전, 서버 공유 토큰·만료·회수·Run 세션은 후속 아키텍처와 검증 목표다. 12절의 날짜별 결과는 당시 버전의 이력이며 이번 변경의 배포 검증 결과를 뜻하지 않는다. 현재 진입점은 자연어와 17개 템플릿이고, 저장 위치는 localStorage이며, 전달 방식은 수신자가 자기 기기에 저장하는 독립된 설정 복사본이다.
...
**사진 처리의 첫 범위:** JPEG·PNG·WebP 입력을 긴 변 기준으로 축소하고 JPEG로 내보낸다. 여러 파일은 ZIP으로 묶는다. 입력은 1~10개, 개별 20MB·전체 50MB·개별 24메가픽셀 이하로 제한한다. 기본 긴 변은 1,600px, 품질은 0.82다. 버전 2.0에서 긴 변은 정수 320~4,096px, 품질은 0.1~1.0이며 이 범위 안의 값을 버튼·도구 실행에 그대로 유지한다. 원본보다 확대하지 않으며 방향 정보를 적용한 뒤 메타데이터를 제거하고, 투명 영역은 흰색으로 합성한다. SVG·GIF·HEIC와 깨진 파일은 변환 전에 안내한다. 출력 파일명은 원본 이름 대신 순번을 사용한다.
...
이 절의 로그인·기록·발행·회수 흐름은 계정 기반 후속 설계다. 현재 구현된 자연어/템플릿 → 편집 → 로컬 저장 → 실행 → 설정 전달 흐름은 16.2절을 따른다.
...
아래 서버 토큰 공유는 후속 요구사항이다. 현재 설정 링크는 16.4절의 독립 복사 모델이며 서버 토큰·만료·회수·신원 확인을 제공하지 않는다.
...
T-01~T-08은 초기 서버 발행·기록 아키텍처의 작업 이력이다. 버전 2.0의 로컬 보관함과 11개 작업 구현 상태·출시 판단은 16절 및 2026-09-14 구현 계획으로 관리한다.
...
1. `날씨`를 입력하고 지역 질문에 `인천`이라고 답해 초안을 만든다. 이름을 바꿔 저장하고 현재 상태와 예보를 실행 화면에서 확인한다.
2. 11개 템플릿에서 버스·사진·체크리스트·타이머 또는 생활 계산 작업을 골라 조건을 바꾼다. 버스의 요일 계산, 사진 조건 복원과 로컬 ZIP, 체크와 타이머 실행을 보여준다.
3. 저장 버튼을 검색·즐겨찾기·복제·수정하고 삭제 후 복원한다. 백업 파일을 새 브라우저에 가져온다.
4. 버튼 설정 링크를 다른 브라우저에서 열고 수신자가 검토해 저장한 뒤 실행한다. 원본과 복사본이 독립적임을 설명한다.
5. 빈 시간표·모호한 터미널·잘못된 파일이나 백업을 넣어 필요한 수정과 기존 데이터 보존을 확인한다.
...
| 2.0 | 2026-09-14: 등록 생활 작업 11종, 작업명만 입력하는 단계별 질문과 템플릿, 편집·검색·즐겨찾기·복제·삭제 취소, 버스·사진 조건 전달, 확장 날씨, UTF-8 백업과 설정 복사 링크, 구버전 로컬 버스 버튼 마이그레이션 구현. 현재 로컬 버전과 계정·운영 후속 범위 분리 |

## 16. 버전 2.0 로컬 생활 버튼 구현 기준

### 16.1 현재 구현 범위와 적용 우선순위

2026-09-14의 구현 단위는 **반복해서 쓸 수 있는 로컬 생활 버튼**이다. 자연어 또는 템플릿으로 초안을 만들고 이름·조건을 편집해 브라우저에 저장하며, 등록된 실행기로 동작하고 백업·설정 복사 링크로 전달한다. 이 절이 현재 제품 동작에 대한 이전 절의 발행·공유 설명보다 우선한다. 계정 기반 상용 서비스의 운영·권한 요구사항은 16.6절의 후속 출시 기준으로 남긴다.

| 작업 종류 | 한국어 작업명 | 입력·범위 | 결과와 실행 위치 |
| --- | --- | --- | --- |
| `weather` | 날씨 확인 | 도시 1~40자 | 서버 Open-Meteo 조회, 현재 상태와 제공되는 오늘 예보·외출 준비 안내 |
| `bus_schedule` | 고속버스 조회 | 출발·도착 이름/ID, 선택 등급·날짜·요일(0~6) | 서버 TAGO 공개 시간표, 10건 단위 페이지 이동 |
| `photo_compress` | 사진 압축 | 긴 변 정수 320~4096px, 품질 0.1~1, 사용자가 선택한 사진 | 브라우저 JPEG 변환과 ZIP 다운로드 |
| `checklist` | 준비물 체크리스트 | 빈 줄 없는 1~20개 항목, 전체 200자 이하 | 체크·진행률·초기화, 기기 내 상태 저장 |
| `timer` | 생활 타이머 | 정수 1~180분 | 종료 시각 기준 남은 시간, 시작·일시정지·이어서 시작·초기화와 로컬 복원 |
| `dday` | 디데이 | 유효한 날짜, 선택 일정 이름 | 한국 날짜 기준 D-DAY·남은 날짜·지난 날짜 |
| `split_bill` | 더치페이 계산 | 원 단위 정수 금액 0~10¹², 인원 정수 1~1000 | 1인당 금액과 나머지 원 단위 분배 |
| `unit_convert` | 단위 변환 | 값 ±10¹² 이내, 같은 종류의 두 단위 | mm·cm·m·km / g·kg / 섭씨·화씨 변환 |
| `text_cleanup` | 글 정리 | 선택 기본 글 200자, 실행 때 최대 20,000자, 공백 정리 또는 중복 제거 | 브라우저 글 정리와 결과 복사 |
| `random_pick` | 무작위 선택 | 서로 다른 후보 최소 2개, 2~20줄, 전체 200자 이하 | 입력한 후보 중 무작위 하나 선택 |
| `counter` | 횟수 기록 | 1회 증감값 정수 1~1000, 기본 1 | 기기 내 횟수 추가·빼기·초기화 |

날씨·버스 이외의 계산과 파일 처리는 기기 내에서 수행한다. 타이머가 화면을 다시 열 때 남은 시간을 복원해도 OS 백그라운드 알람이나 닫힌 페이지의 종료 알림을 보장하지 않는다. 실제 사진과 계산 대상 글을 AI로 보내야 실행할 수 있는 구조가 아니다.

### 16.2 생성·질문·편집·실행

1. `/create`에서 자연어를 입력하거나 11개 템플릿을 직접 선택한다. 템플릿 선택은 AI 호출 없이 동일한 `create_button` 초안을 만든다.
2. `날씨`·`버스`·`타이머`·`더치페이` 등 등록 작업명만 입력해도 생성 요청으로 해석한다. 도시·출발지·도착지·분·금액·인원 등 빠진 필수값을 한 항목씩 묻는다. `날씨 → 인천`, `타이머 → 25분` 같은 답변은 앞선 요청에 누적한다. 사용자가 말하지 않은 도시나 날짜를 임의로 채우지 않는다.
3. 짧은 작업명과 등록 단축 규칙은 결정적으로 처리하며, 더 자유로운 자연어는 서버 Gemini 해석을 사용한다. 잘못된 숫자·범위 답변은 해당 입력을 다시 묻는다.
4. 사용자는 부족한 값을 지금 입력하거나 실행할 때 물어보는 입력으로 남길 수 있다. 초안과 저장 버튼 모두 이름·고정 조건·실행 입력을 편집하고 계약 검사를 통과한 뒤 저장한다. 작업 설명은 초안의 값을 유지한다.
5. 보관함에서 실행하면 필요한 값만 요청한다. 날씨·체크리스트·타이머·생활 계산은 결과 화면에서 사용하고, 버스·사진은 검증한 조건을 전용 도구로 넘긴다. 반복 실행 시 Gemini는 호출하지 않는다.

생성·편집·파일·링크의 입력은 `parseButtonIntent`로 11개 작업의 허용 필드·타입·범위를 검사한다. 사진 파일은 버튼에 고정하지 않고 사진 도구에서 직접 선택한다. 임의 URL·코드·로그인·송금·결제를 실행 작업으로 받아들이지 않는다.

### 16.3 보관함과 구버전 호환

- 저장 키는 `hanbeonman.generated-buttons`이며 기존 배열 형식을 유지한다. 새 ID는 UUID를 사용하고 `favorite`, `updatedAt`은 선택 메타데이터다.
- 최대 50개 버튼을 저장하며 이름·설명·조건 검색, 작업 종류 필터, 즐겨찾기 우선 표시, 수정·복제·삭제·마지막 삭제 취소를 지원한다.
- 저장·복제·복원으로 한도를 넘으면 기존 항목을 자동 삭제하지 않는다. 실제 `localStorage` 쓰기가 실패하면 저장 성공으로 표시하지 않고 기존 자료를 유지한다.
- 구버전 `generated-숫자` ID의 버스 버튼에 출발·도착이 빠졌다면 해당 터미널을 필수 실행 입력으로 보충한다. 기존 라벨·고정 조건·ID·시각·즐겨찾기는 보존하고 보충한 결과도 현재 계약으로 검사한다.
- 마이그레이션은 로컬 구버전 읽기에만 적용한다. 가져오는 백업과 설정 링크는 현재 계약을 엄격하게 적용하며 손상된 배열·작업·입력·중복 ID를 허용하는 이유로 사용하지 않는다.
- 체크·타이머·횟수 상태는 버튼별 로컬 실행 데이터로 저장한다. 같은 기기·브라우저의 저장이며 계정이나 다른 기기의 자동 동기화가 아니다.
...
```json
{ "format": "hanbeonman-buttons", "version": 1, "buttons": [] }
```

가져오기·내보내기와 로컬 직렬화는 UTF-8 기준 200,000바이트 제한을 적용한다. 파일 크기는 읽기 전에도 확인한다. 모든 항목이 유효해야 가져오며, 동일 ID는 기존 버튼을 유지하고 새 ID만 합친다. 합친 결과가 50개를 넘거나 저장이 실패하면 기존 자료를 유지한다. 백업에는 버튼 설정과 보관함 메타데이터만 담고 사진·체크·타이머·횟수 상태는 포함하지 않는다.

설정 링크는 `/create#button=` 뒤에 URL 인코딩한 `create_button` JSON을 넣는다. 인코딩된 fragment는 약 16KB 상한으로 검사한다. fragment는 웹페이지가 클라이언트에서 읽으며 수신자가 내용을 검토하고 저장하기 전 실행하지 않는다. 링크에는 버튼 제목·설명·조건이 들어가고 저장 ID는 전달하지 않는다.

받은 설정은 **수신자가 자기 기기에 저장하는 독립된 복사본**이다. 발신자의 원본 수정이 반영되지 않으며 원격 회수·만료·권한 토큰·수신자 신원 확인 기능은 없다. 이 링크를 9.1절의 서버 관리 비공개 공유 링크로 표현하지 않는다. 복사 기능은 링크를 클립보드에 준비하며 메신저로 자동 전송하지 않는다.

### 16.5 버스·사진·날씨 연결 계약

버스 실행 URL은 `/bus?departure=…&arrival=…&grade=…&weekday=0..6&date=YYYY-MM-DD&auto=1`이다. 제공된 터미널 이름·ID를 목록에서 정확히 찾고 서울→서울경부·대전→대전복합의 명시적 별칭만 추가 해석한다. 모호한 이름은 임의 첫 항목을 선택하지 않는다. 없는 등급을 전체 등급으로 바꾸지 않고 사용자가 다시 선택할 때까지 자동 조회를 중단한다.

유효한 명시 날짜를 우선하며, 없으면 한국 시간 기준 오늘을 포함한 가장 가까운 요일을 계산한다. 이번 주에 지난 요일은 다음 주로 넘기고 날짜 조건이 없으면 오늘을 쓴다. 잘못된 날짜·요일은 확인을 요청한다. 터미널·등급 목록 로드와 조건 검증이 끝났을 때만 자동으로 한 번 조회한다.

사진 실행 URL은 `/photo?maxEdge=…&quality=…`이다. 유효한 옵션을 복원하고 범위 밖 값은 오류와 기본값을 표시한다. 사진 선택 전에는 변환하지 않으며 모든 파일 처리는 브라우저 안에서 수행한다.

날씨는 기존 상태·기온·강수량을 유지하며 체감온도·습도·바람·오늘 최저/최고·강수확률·관측/조회 시각을 제공되는 범위에서 표시한다. 공급자가 일부 선택 필드를 주지 않아도 현재 날씨 조회를 유지하고 없는 예보 수치를 만들어내지 않는다. 외출 준비 안내는 실제 반환된 자료에 근거한다.

### 16.6 계정 기반 출시 후속 조건

| 후속 범위 | 공개 출시 판정에 필요한 구현과 검증 |
| --- | --- |
| 계정과 동기화 | Supabase Auth 로그인·복구, owner_id RLS, 로컬 자료의 선택적 계정 이관, UUID·updatedAt 기반 충돌 처리와 다중 기기 검증 |
| 회수 가능한 공유 | 서버 토큰·발행 버전·만료·회수와 권한 검증. 현재 독립 설정 링크와 구분 |
| 탈퇴와 삭제 | 계정 탈퇴, 관련 데이터 삭제·보관 기간, 사용자 확인과 복구 정책 |
| API 운영 | 분산 배포에서도 유지되는 사용량 제한, 오류 관측, 공급자 비용 상한·장애 대응 |
| 실제 사용자 | 가족의 독립 완료율과 반복 사용, 설정·복구를 포함한 도움 시간, 실제 외부 조회 조건별 품질 |

Supabase 테이블·RLS가 이미 있어도 현재 로컬 보관함에 인증·서버 CRUD·동기화가 연결됐다고 표시하지 않는다. 자동화 테스트는 실제 가족 관찰이나 사업성 검증을 대체하지 않는다.

### 16.7 구현·검증·배포 기록

2026-09-14에 11개 작업 계약·단계별 질문·템플릿·보관함·실행·백업·설정 링크를 구현하고 통합 검증했다. 전체 단위 테스트 214개, 전체 Playwright 기준선 102개가 통과했다. 이후 여러 줄 입력의 접근성 라벨 수정 회귀 3개를 데스크톱·모바일·320px에서 추가로 통과시켜 총 105개 시나리오를 검증했다. 타입 검사·프로덕션 빌드·비밀값 검사, 비밀 검사기 테스트 5개와 Supabase 스키마 검사 1개가 통과했고 접근성 수정 후 빌드도 다시 통과했다.

기능 커밋 `5c8ed8b`(날씨), `e0e5966`(버스·사진), `3a2b844`(작업 계약), `1d04e8c`(보관함·Gemini), 후속 수정 `f6d0f5d`를 푸시했다. Production `/create`에서 11개 템플릿과 `날씨 → 인천 → 저장 → 날씨 실행`을 실제 브라우저로 확인했고 해당 흐름에서 페이지 오류가 없었다. 실제 Gemini의 인천 날씨 요청은 200과 날씨 초안을, 금요일 서울→대전 요청은 200과 `departure:서울`, `arrival:대전`, `weekday:5`를 반환했다. 실제 날씨 API도 200과 추가 날씨 필드·오늘 예보를 반환했다. Production TAGO 목록에는 `대전복합`이라는 이름으로 `NAEK300`·`NAEK301`이 있어 일반 `대전` 요청에 사용자 선택을 요구하는 것을 확인했다. 명시 ID `NAEK010 → NAEK300`, 금요일 조건은 `depPlandTime=20260918`로 자동 조회되어 HTTP 200·`EMPTY`·0건을 반환했다. 이는 조건 전달·빈 결과 처리 확인이며 해당 날짜의 운행편을 찾았다는 뜻은 아니다. 이 점검은 11개 모든 작업의 Production 실사용이나 가족 사용자 관찰을 완료했다는 뜻은 아니다.

- [x] 등록 작업 11종과 보관함·편집·로컬 저장·실행·백업·설정 링크 구현
- [x] 구버전 버스 입력 보충, UTF-8 크기 제한, 잘못된 입력과 저장 실패의 회귀 테스트 추가
- [x] 전체 단위 테스트·타입 검사·프로덕션 빌드·비밀값 검사 결과 기록
- [x] 데스크톱·모바일·320px E2E와 여러 줄 입력 접근성 회귀 결과 기록
- [x] 기능별 한국어 커밋·푸시와 Production 주요 API·생성·실행 화면 점검 기록
- [ ] 16.6절의 계정·운영 출시 조건과 실제 사용자 검증 완료


## 17. 버전 2.1 생활 버튼 6종 확장

현재 등록 도구는 기존 11종에 아래 6종을 더한 **17종**이다. 16절의 로컬 보관함·백업·설정 링크 계약을 유지한다. 이 절은 추가 기능에 관해 16절보다 우선하며, 16.7절의 과거 검증 기록은 당시 버전의 결과로 보존한다.

| actionKind | 입력 계약 | 실행 |
| --- | --- | --- |
| qr_code | text 필수, 1~200자 | qrcode를 동적 로드해 기기 내 320px PNG 생성, 미리보기·다운로드·실패 재시도 |
| directions | destination 필수, origin 선택, 각각 1~100자; mode 선택(transit/driving/walking/bicycling) | 고정 Google Maps HTTPS 주소에 api=1·목적지·선택 출발지·이동 수단을 URL 인코딩해 이동. 기본 대중교통. URL 최대 2,048자 |
| text_copy | text 필수, 1~200자 | 실행 시 저장한 글 복사, 권한 차단 시 결과 글 선택·직접 복사·재시도. 실행마다 한 번 시도 |
| discount | price 정수 0~10¹²원, rate 0~100 | 소수 할인율을 정확히 계산해 최종 금액을 원 단위 반올림, 정가와의 차이를 할인 금액으로 표시 |
| unit_price | priceA/B 정수 0~10¹²원, quantityA/B 0.001~10⁹, 공통 unit은 g/ml/개 | 100g·100ml·개당 가격을 비교. 동률 판정은 표시 반올림 전 값으로 계산 |
| recipe_scale | baseServings/targetServings 0.1~1,000, ingredients 빈 줄 없는 1~20개·200자 이하 | 재료명·양·단위를 파싱해 인분 비율로 환산. 소수·분수 지원, 오류 줄을 알리고 실행 입력 유지 |

QR·복사·계산 작업은 외부 서비스에 내용을 보내지 않고 브라우저에서 처리한다. 자연어로 버튼을 생성하는 별도 요청은 기존 Gemini 해석 정책을 따른다. QR코드는 입력 내용을 담는 이미지이며 링크에 자동 접속하지 않는다. 길찾기는 Google 지도에 조건을 전달하며 지도 공급자의 지원 지역·교통 수단에 따라 경로가 제공된다.

작업명만 입력하는 단계별 질문도 확장했다. 예를 들어 QR코드→링크, 주소 복사→복사할 글, 할인 계산→정가→할인율, 단가 비교→A 가격→A 수량→B 가격→B 수량, 레시피 분량→기준 인분→목표 인분→재료 목록으로 진행한다. 잘못된 답은 기존 유효값을 보존하고 다시 묻는다. 명확한 강남역 길찾기, 5만원 20% 할인은 단축 규칙으로 바로 생성한다. 복잡한 문장은 Gemini의 전체 작업 계약으로 해석한다.

도구 목록에는 이름·설명 검색과 전체/외출·이동/일상·루틴/쇼핑·계산/사진·공유 필터가 있다. 검색 결과가 없으면 검색·분류를 함께 초기화할 수 있다. UI의 도구 수는 카탈로그 길이로 계산한다.

참조: [Google Maps URLs 공식 문서](https://developers.google.com/maps/documentation/urls/get-started), [node-qrcode 공식 저장소](https://github.com/soldair/node-qrcode).


### 17.1 검증·배포 결과

단위 테스트 28개 파일·285개와 타입 검사가 통과했다. 전체 Playwright 126개를 통과한 뒤 레시피 오류 입력 보존 회귀 3개를 추가했다. 수정 후 신규 도구 테스트 24개를 PC·모바일·320px에서 재검증해 전체 고유 시나리오129개를 확인했다. 프로덕션 빌드·비밀값 검사·diff 검사도 통과했다. 지도 장소 길이는 계약·UI·실행 helper 모두100자로 맞췄다.

기능별로 `eb5b803`(계산 도구3종), `61daf60`(QR·복사·지도 helper), `d2f00e8`(계약·자연어·17종 카탈로그·검색·분류·실행 연결)을 커밋·푸시했다.

Production `/create`에서17개 템플릿을 확인했다. QR코드·길찾기·주소 복사·단가 비교·레시피 분량의 짧은 요청은 HTTP200과 부족한 정보 질문을 반환했고, `5만원 20% 할인`은 price50000/rate20을 반환했다. 실제 Gemini에 `https://example.com 링크를 QR코드로 만들어주는 버튼 만들어줘`를 보내 qr_code와 해당 text를 받았다. 실제 모바일 브라우저에서 QR 버튼 저장→내용 입력→320px 이미지 생성→`hanbeonman-qr.png` 다운로드를 완료했으며 페이지 오류·가로 넘침이 없었다. 지도 이동은 브라우저 테스트에서 공식 URL의 목적지·교통수단 전달을 확인했으며 특정 실제 경로 제공 여부를 검증했다는 의미는 아니다.
