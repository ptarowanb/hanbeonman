...
| 버전 | 2.0 |
| 작성일 | 2026-09-14 |
| 상태 | 11개 생활 작업·짧은 작업명 질문·템플릿·초안 및 저장 버튼 편집·로컬 보관함·실제 실행 연결·백업·설정 복사 링크 구현 및 통합 검증·Production 주요 흐름 점검 완료. 검증 범위는 16.7절을 따르며 Supabase 인증·동기화·서버 공유 토큰·공개 출시 운영과 실제 사용자 검증은 후속 범위 |
...
**버전 2.0 적용 순서:** 현재 사용자에게 제공하는 동작과 완료 판정에는 [16절](#16-버전-20-로컬-생활-버튼-구현-기준)이 우선한다. 1~14절에 남긴 Chrome 기록, Supabase 소유권·불변 발행 버전, 서버 공유 토큰·만료·회수·Run 세션은 후속 아키텍처와 검증 목표다. 12절의 날짜별 결과는 당시 버전의 이력이며 이번 변경의 배포 검증 결과를 뜻하지 않는다. 현재 진입점은 자연어와 11개 템플릿이고, 저장 위치는 localStorage이며, 전달 방식은 수신자가 자기 기기에 저장하는 독립된 설정 복사본이다.
