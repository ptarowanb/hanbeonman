...
| 버전 | 0.1 |
| 작성일 | 2026-09-09 |
| 상태 | 구현 전 설계 기준. 제품 성능과 사업 가설은 미검증 |
...
**사진 처리의 첫 범위:** JPEG·PNG·WebP 입력을 긴 변 기준으로 축소하고 JPEG로 내보낸다. 여러 파일은 ZIP으로 묶는다. 입력은 1~10개, 개별 20MB·전체 50MB·개별 24메가픽셀 이하로 제한한다. 기본 긴 변은 1,600px, 품질은 0.82다. 원본보다 확대하지 않으며 방향 정보를 적용한 뒤 메타데이터를 제거하고, 투명 영역은 흰색으로 합성한다. SVG·GIF·HEIC와 깨진 파일은 변환 전에 안내한다. 출력 파일명은 원본 이름 대신 순번을 사용한다.
...
웹 UI·API·공통 계약은 TypeScript로 구성하는 안을 기준으로 한다. 웹 프레임워크는 Next.js, 제작 기록은 Chrome Manifest V3 확장, 사진 처리는 브라우저 Worker와 이미지 변환·ZIP 라이브러리 조합을 제안한다. 아직 설치되거나 동작 검증된 기술 스택은 아니다. 구현 시점에 호환 버전과 라이선스를 확인한다.

소유자·명세·공유·실행 상태는 관계형 저장소에 보관하고, 소유자 인증은 서버 세션으로 검증한다. 공개 조회는 제한 시간 안에 끝나는 서버 작업으로 구현한다. 별도 큐나 브라우저 실행 서버는 실제 어댑터가 요구할 때 도입하며 MVP의 기본 전제로 두지 않는다.

예상 구현 경계는 `apps/web`, `apps/recorder`, `packages/contracts`, `packages/connectors`다. 현재 생성된 디렉터리나 실행 명령을 뜻하지 않는다.
...
| --- | --- | --- |
| 1. 연동·데이터 확인 | 공개 조회 계약과 샘플/실제 모드 구분 | 공식 출처의 실제 응답·범위 확인. 실패 시 실사용 연동 미완료로 기록 |
| 2. 공통 계약 | 이벤트·명세·버전·공유·실행 스키마와 Validator | 잘못된 참조·동작·권한 확대 거부 |
| 3. 생성·검토 | 두 제작 환경과 AI 초안·수신자 미리보기 | AC-01~05 확인 |
| 4. 실행·공유 | 서버 조회, 로컬 사진 처리, 비공개 링크 | AC-06~14 확인 |
| 5. 사용자·운영 검증 | 모바일 관찰, 데이터 모드·보관 검증 | AC-15~17 확인 및 실패 사항 해결 |
