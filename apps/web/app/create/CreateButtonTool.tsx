...
export default function CreateButtonTool() {
  ...
  return <>
    <nav className="library-jump" aria-label="버튼 화면 바로가기"><a href="#saved-buttons">내 버튼 {buttons.length}개</a><a href="#action-templates">11가지 생활 작업</a><span>로그인 없이 이 기기에 저장</span></nav>
    <section className="create-tool" aria-labelledby="create-tool-title">
      <div className="create-tool-heading"><p className="eyebrow">내 말로 만드는 생활 버튼</p><h2 id="create-tool-title">“날씨” 한마디로<br/><em>시작해도 괜찮아요.</em></h2><p>필요한 정보만 물어보고, 다음에도 쓸 수 있는 버튼으로 만들어드려요.</p></div>
      <form className="create-form" onSubmit={submitRequest}><label htmlFor="button-request">만들고 싶은 작업</label><textarea id="button-request" value={request} onChange={event=>setRequest(event.target.value)} placeholder="날씨 / 25분 타이머 / 금요일 서울에서 대전 가는 버스" maxLength={1000}/><div className="create-form-footer"><span>{request.length}/1,000</span><button className="create-submit" type="submit" disabled={isLoading}>{isLoading?"만드는 중…":"버튼 만들기"}</button></div></form>
    ...
    <section id="action-templates" className="library-templates" aria-labelledby="templates-title"><p className="eyebrow">매일 쓸 수 있는 11가지</p><h2 id="templates-title">고르고, 내 방식으로 바꿔보세요.</h2><p>이름과 조건을 바꾸면 나만의 버튼이 됩니다. 직접 선택할 때는 AI를 기다릴 필요가 없어요.</p><div className="library-template-grid">{ACTION_KINDS.map(kind=><button key={kind} type="button" aria-label={`${ACTIONS[kind].label} 템플릿`} onClick={()=>showDraft(makeTemplate(kind))}><span className="library-symbol" aria-hidden="true">{ACTIONS[kind].symbol}</span><strong>{ACTIONS[kind].label}</strong><small>{ACTIONS[kind].description}</small><span className="library-template-cta">내 버튼 만들기 ↗</span></button>)}</div></section>
    <section id="saved-buttons" className="create-saved library-saved" aria-labelledby="saved-buttons-title">
      <div className="create-saved-heading"><div><p className="eyebrow">내가 만든 버튼</p><h2 id="saved-buttons-title">다음에는<br/>여기서 누르세요.</h2></div><span>{buttons.length}/{MAX_BUTTONS}</span></div>
      ...
      {!loaded?<p>저장한 버튼을 불러오고 있어요…</p>:visible.length?<ul className="library-button-grid">{visible.map(button=><li key={button.id}>
        <button className="library-run-button" type="button" aria-label={`${button.title} 실행`} onClick={()=>{setRunning({...button});setRunVersion(value=>value+1);setNotice("");}}><span className="library-button-kind">{ACTIONS[button.actionKind].symbol} {ACTIONS[button.actionKind].label}</span><strong>{button.title}</strong><span>{actionSummary(button)}</span><small>한 번 눌러 실행 ↗</small></button>
